// LLM Assistant for enhanced health plan analysis and insights
...
export class LLMAssistant {
  constructor() {
    ...
    // Auto-configure with system defaults if available
    this.autoConfigureFromEnv();
  }

  async autoConfigureFromEnv() {
    const isEnabled = await EnvConfig.isLLMEnabled();
    if (!isEnabled) {
      console.log('🚫 LLM features disabled by configuration');
      return;
    }

    const bestConfig = await EnvConfig.getBestConfig();
    if (bestConfig) {
      this.configure(
        bestConfig.apiKey, 
        bestConfig.provider, 
      ...
      if (bestConfig.isSystemDefault) {
        console.log(`🤖 Auto-configured LLM with system defaults: ${bestConfig.provider} (${bestConfig.model})`);
      } else {
        console.log(`🤖 Auto-configured LLM with user settings: ${bestConfig.provider} (${bestConfig.model})`);
      }
    }
  }
  ...
  async analyzeSBCText(rawText, fileName) {
    if (!this.isConfigured) {
      console.log('LLM not configured, using fallback analysis');
      return null;
    }
...

{
  ...
  "individualDeductible": "Amount in dollars (number)",
  "familyDeductible": "Amount in dollars (number)", 
  "individualOOPMax": "Individual out-of-pocket maximum (number)",
  "familyOOPMax": "Family out-of-pocket maximum (number)",
  "primaryCopay": "Primary care copay amount (number)",
  "specialistCopay": "Specialist copay amount (number)",
  "urgentCareCopay": "Urgent care copay (number)",
  "emergencyRoomCopay": "ER copay (number)",
  "coinsurance": "Coinsurance percentage as decimal (e.g., 0.20 for 20%)",
  "rxDeductible": "Prescription drug deductible if separate (number)",
  "tier1DrugCost": "Tier 1/Generic drug copay (number)",
  "tier2DrugCost": "Tier 2/Preferred brand drug copay (number)", 
  "tier3DrugCost": "Tier 3/Non-preferred brand drug copay (number)",
  "specialtyDrugCost": "Tier 4/Specialty drug copay or coinsurance (number)",
...
- For drug costs, if they show percentages (like "30%"), convert to decimal (0.30)
...
Keep each section under 50 words. Focus on practical, actionable advice.`;
  ...
  parseSBCResponse(response) {
    try {
      ...
      console.warn('Could not parse LLM response as JSON:', response);
      return null;
    } catch (error) {
  ...
  static showConfigurationModal() {
    const config = EnvConfig.getConfig();
    const hasSystemDefaults = EnvConfig.hasSystemApiKeys();
    
    const modal = document.createElement('div');
    modal.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
    modal.innerHTML = `
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <h3 class="text-lg font-semibold mb-4">🤖 Configure AI Analysis</h3>
        ...
        ` : `
          <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p class="text-sm text-blue-700">
              <strong>Optional:</strong> Add your own OpenAI or Anthropic API key to get enhanced plan analysis, 
      ...
      modal.addEventListener('click', (e) => {
        if (e.target.matches('[data-action="skip"]') || e.target === modal) {
          ...
          if (hasSystemDefaults) {
            const bestConfig = EnvConfig.getBestConfig();
            resolve(bestConfig);
          } else {
            resolve(null);
          }
